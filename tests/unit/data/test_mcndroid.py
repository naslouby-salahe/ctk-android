...
def test_shards_cover_every_split_of_every_representation(tmp_path: Path) -> None:
    ...
    for kind in representations.kinds():
        assert len(representations.shards(paths, kind)) == len(SPLITS)
    assert len(representations.source_files(paths)) == 2 * len(SPLITS) * 2 + len(SPLITS)
...
def test_hashes_follow_shard_order_for_every_key_spelling(tmp_path: Path) -> None:
    ...
    for kind in representations.kinds():
        assert representations.kind_hashes(paths, kind).to_list() == expected
...
def test_only_wanted_rows_are_loaded_and_alignment_restores_the_requested_order(
    ...
    for kind, width in widths.items():
        block = representations.load_wanted(paths, kind, wanted)
        assert block.matrix.shape == (3, width)
        assert set(block.shas.to_list()) == set(wanted.to_list())
        positions = representations.alignment(block, wanted, kind)
...
def test_alignment_rejects_rows_missing_from_the_shards(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    block = representations.load_wanted(
...
def test_source_fingerprint_hashes_every_file_and_changes_with_content(tmp_path: Path) -> None:
    paths = _paths(tmp_path)
    scan = representations.fingerprint_sources(paths, SourceInventory(entries=()))
    assert scan.fingerprint.file_count == len(representations.source_files(paths))
    assert (
        scan.fingerprint == representations.fingerprint_sources(paths, scan.inventory).fingerprint
    )
    target = representations.source_files(paths)[0]
    target.write_bytes(target.read_bytes() + np.zeros(1, dtype=np.uint8).tobytes())
    changed = representations.fingerprint_sources(paths, scan.inventory)
