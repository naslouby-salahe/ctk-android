...
from ctk_android.data import cache, preparation, representations
...
def test_overlap_keeps_rows_present_in_every_source_in_lamda_order() -> None:
    ...
    kept = representations.overlap_table(assignments, both)
...
def test_restricted_identities_keep_component_membership_with_dense_ids() -> None:
    ...
    full = preparation.build_identities(assignments.with_columns(packages), features)
    overlap = representations.overlap_table(assignments, [pl.Series([sha(1), sha(3), sha(4)])])
    restricted = representations.restrict_identities(full, overlap)
...
def test_signed_log_keeps_sign_and_compresses_magnitude() -> None:
    ...
    transformed = representations.signed_log(sp.csr_matrix(dense.astype(np.float32))).toarray()
...
def test_csr_round_trip_preserves_structure_and_precision(tmp_path: Path) -> None:
    ...
    for kind, half in ((Representation.CALL_GRAPH, False), (Representation.REPORT_JSON, True)):
        files = representations.csr_files(paths, kind)
        representations.save_csr(files, sp.csr_matrix(dense), half=half)
        restored = representations.load_csr(files, 3)
        np.testing.assert_array_equal(restored.toarray(), dense)
    assert (
        np.load(representations.csr_files(paths, Representation.REPORT_JSON).data).dtype
        == np.float16
    )
    assert (
        np.load(representations.csr_files(paths, Representation.CALL_GRAPH).data).dtype
        == np.float32
    )
...
def test_binary_rows_reads_the_requested_positions_as_bytes() -> None:
    ...
    rows = representations.binary_rows(matrix, np.array([2, 0]))
...
def test_namespace_aligns_every_representation_to_the_overlap_rows(tmp_path: Path) -> None:
    ...
    graph = representations.load_csr(
        representations.csr_files(paths, Representation.CALL_GRAPH), manifest.graph_features
    )
    np.testing.assert_allclose(
        graph.toarray(), np.stack([written.graph[index] for index in OVERLAP])
    )
    assert all(record.passed for record in representations.namespace_validations(paths, manifest))
    assert (
        cache.read_record(paths.representation_file(Artifact.OVERLAP_MANIFEST), OverlapManifest)
        == manifest
    )
    report = representations.load_csr(
        representations.csr_files(paths, Representation.REPORT_JSON), manifest.json_columns_total
    )
...
def test_studies_load_raw_representations_and_leave_fitting_to_training(
    ...
    representations.build_namespace(paths)
    ...
    for name, width in expected_widths.items():
        ...
        study = representations.load_study(paths, CONFIG, spec, KEY)
        ...
        rule = representations.transform_rule(spec.representation, CONFIG)
        fitted = spec.representation in (Representation.CALL_GRAPH, Representation.REPORT_JSON)
        assert (rule is not None) == fitted
    json_rule = representations.transform_rule(Representation.REPORT_JSON, CONFIG)
