...
H5 is compound. The dose-response gate (31.2) tests only the first part (benefit grows with effective peer exposure); it does not test diminishing returns. The confirmatory evidence supports the increase only. The prospective exact-dose extension is consistent with diminishing marginal returns, but no plateau is observed through 200 exact peer samples (see 31.3 M).
...
**F. Feature novelty.** The gate outcome is preserved as stored. Gate non-passage with very few families and wide intervals is unresolved evidence, not a demonstrated absence of an association; prose must not say novelty is disproven or proven. Other descriptors are exploratory only. Cross-reference: the separately frozen prospective larger-family study EXT-LFAM (31.4, class D, 32 families, seeds 300 to 309) reports the same descriptor (Spearman rho 0.39, 95% interval 0.06 to 0.65); it is a separate class D result and does not change the original gate status.
...
**L. Per-client CTK analysis (class C).** For each of the four clients, the decomposition (local, no-family, peer-family and same-arm full-exposure recall; total, pooling and CTK gain; known-family change; realised FPR; support; contributing seeds; eligible family pairs) is computed from stored client-level metrics for FedAvg, FedProx and centralized arms at the primary operating point, federation-wide and own-domain. It uses existing results only, creates no gate, is descriptive and exploratory, and does not alter any original confirmatory outcome. Every client is reported with its number of contributing seeds. Intervals are exploratory small-n BCa intervals over the contributing seeds, shown whenever the existing BCa helper can compute one (at least 3 seeds and a non-constant effect; an implementation requirement, not a scientific threshold) and omitted otherwise. With 6 to 10 seeds and unequal per-seed support such intervals are unstable and understate uncertainty; they are descriptive. The gate's 8-of-10 positive-seed rule is not an interval-validity criterion and is not used for this purpose. With four clients, associations with client properties are descriptive and not causal.

**M. Dose interpretation.** The promoted confirmatory dose gate supports that recall improves as effective peer exposure increases; that gate alone does not establish diminishing marginal returns or saturation. The prospective exact-dose study EXT-DOSE (31.4, class D, seeds 310 to 319) is consistent with diminishing marginal returns but observes no plateau through 200 exact effective peer samples. Both conclusions must be kept distinct wherever H5 is cited; the original dose gate outcome (promoted) is unchanged.

**N. Novelty positioning.** After the full-text literature audit in `docs/Research Evidence.md` (section 29; final identification-based rule, 41-row collision matrix: 0 DIRECT, 4 PARTIAL, 34 ADJACENT, 3 NONE; Popoola et al. is ADJACENT because its local comparison is not class-specific), the closest collision remains CyberForce, which pairs an unmatched absent-everywhere arm with peer-present arms, a peer-holder sweep, local curves and robust aggregators on the absent malware. FID-SPA and FEDroid full texts remain unavailable; FedRS, FedLMD, FedCKD, Ciaramella et al., Liu et al. Cybersecurity 2026 (prose only) and Ishfaq et al. PLOS One 2026 zero-day were read in full text or main text and verified, among others. The residual contribution is a measurement design and empirical decomposition on one corpus, with "no closely matching study found in the audited literature" wording restricted to the combination; the dose analysis and natural-scarcity check are partially anticipated. The audit is limited and does not license priority language.

**O. Family x client x seed variance decomposition (class C).** REML crossed random effects (family, client, family-by-client, seed) on per-cell FedAvg CTK, from stored runs (`ctk-heterogeneity-components`). Primary set: the client component is estimated at 0.59 of own-domain variance (four clients) and family at 0.63 of federation-wide variance; seed point shares are below 0.10 (upper bounds up to 0.26). The replication set has no dominant component. Four clients, wide bootstrap intervals over seeds; descriptive.

**P. Aggregate-metric masking (class C).** Paired per-seed differences in AUROC/AUPRC versus unseen-family recall at 5% FPR (`aggregate-metric-masking`, 64 rows). Against local, FedAvg changes AUROC by -0.012 and AUPRC by -0.020 while federation-wide hidden-family recall rises by +0.132. 15 of 64 rows (8 distinct recall contrasts, each counted once per aggregate metric) are flagged as masked gains (6 primary, 9 replication) and none as a masked loss. The flag reuses the +0.03 practical threshold and is not a gate.

**Q. Negative transfer: repair versus new capability (class C).** Per (client, hidden family) cell, repair = min(max(CTK, 0), max(-pooling, 0)) and new capability = max(CTK, 0) - repair (`negative-transfer-decomposition`; definitions in `src/ctk_android/analysis/hidden_family.py`). Own-domain, primary set: repair is 59% of gross positive CTK (federation-wide 26%). Repair is an accounting bound, not a causal effect, and truncation at zero inflates both parts under sampling noise.

O, P and Q are recomputed from stored runs only, use class C labels, alter no gate and no original estimate, and are produced by `uv run ctk-android posthoc --promote`.
...
- Interval inside the band: the own-domain null control is resolved as clean; the original caveat is reported as "unresolved in v1, resolved prospectively in EXT-1" and the own-domain causal wording is strengthened accordingly.
...
**EXT-1 result.** The primary endpoint was met: own-domain FedAvg permutation CTK at 5% FPR is +0.006 (-0.004, 0.014), inside the band. The original own-domain caveat is reported as unresolved in v1 and resolved in the extension; original results and gates are unchanged (`docs/Research Evidence.md` section 20.1). Literature-justified extensions not run (missing-class baselines, real placebo family, KronoDroid, McNdroid, exact-dose, larger-family novelty, new mechanism) and the reasons are in section 33 of that document. Status update: the placebo, robust-aggregation, exact-dose, larger-family and McNdroid representation extensions were subsequently frozen and run as the extension-b protocols below; the others were not run.

**Extension-b protocols (class D, frozen 2026-09-25 before any run in mode `extension-b`).** Four further prospective protocols, kept separate from the original study and from EXT-1. They use mode `extension-b`, fresh seeds EXT-LFAM 300-309, EXT-DOSE 310-319, EXT-CTRL 320-329 and EXT-REP 330-339 (seeds now 300-339), are never pooled with 100-109, 200-209 or with each other, and cannot change the 9 promoted / 0 narrowed / 3 rejected outcome. Protocol texts (working copies in `docs/temp/extension-protocol/`) are hashed in `FREEZE.md` (freeze manifest, 2026-09-25T11:07:34+00:00). Commands: `large-family`, `dose-extension`, `controls-extension`; outputs in `results/extension-b/`.

*EXT-LFAM (larger-family explanation, seeds 300-309).*
- Question: why do some hidden families gain more from peer knowledge (H7 had 7 and 8 families, rho 0.57 and 0.33, intervals spanning zero)?
- Hypothesis H-LFAM: Spearman rho between the single predeclared descriptor `nearest-known-family-distance` (as in `analysis/novelty.py`) and family-mean FedAvg CTK is positive with the lower bound of its family-cluster bootstrap interval above 0.
- Population: fixed from the seed-000 partition without outcomes: 32 families with at least one eligible (client, family) pair (79 pairs; 29 with own-domain support), ranked by rows and split into four batches of eight (`large-family-set-1..4`, hidden simultaneously as in controlled exposure); the 15 families already used are re-measured, the 17 new ones reported separately.
- Arms: identical to `controlled-exposure` (local, central, FedAvg; peer-present, family-absent-everywhere, full exposure). Estimands: family-mean federation-wide CTK at 5% FPR; own-domain, total gain and local recall secondary.
- Controls/statistics: Spearman over 32 families, 9,999-resample family-cluster bootstrap, exact permutation p; minimum detectable |rho| about 0.48 at 80% power, stated in advance; development seeds 1-5 used only for identifiability and not evidence.
- Interpretation (fixed): interval above 0 = supported moderate explanatory variable, never causal; interval spanning 0 with estimate 0.3 to 0.5 = unresolved and underpowered; below 0.3 near 0 = not useful; significantly negative = contradicts H7 direction.
- Result: rho = 0.39 (95% interval 0.06 to 0.65) over 32 families; permutation p = 0.026 (9,999 Monte Carlo permutations; 32! orderings cannot be enumerated; the asymptotic p 0.027 was reported before the final verification pass and labelled exact in error); seed-level rho median 0.22 (range -0.06 to 0.43, 9 of 10 positive). H-LFAM met on the interval rule with the frozen power caveat (interval width 0.59). Eligibility-stability sensitivity (no training): 27 of 32 families eligible in all 10 fresh seeds; on them rho 0.27 (-0.10, 0.58), so the interval verdict depends on the five families with incomplete eligibility; the frozen result stays primary. Class D; the original feature-novelty gate stays "rejected".

*EXT-DOSE (exact effective dose, seeds 310-319).*
- Question: how does CTK change with the exact number of peer-held samples, and is there onset and saturation (the original dose gate delivered far fewer effective samples than requested)?
- Hypotheses: H-DOSE-1 CTK(d) > 0 and rises with d; H-DOSE-2 the 100 to 200 increment lies inside +/-0.03.
- Population: primary (7) and replication (8) sets, hidden (target, family) pairs under eligibility profile `dose` (primary rule with peer_min_fit 200).
- Arms (FedAvg; central secondary): family-absent-everywhere training set with exactly d in {0, 10, 25, 50, 100, 200} rows of the family substituted for randomly chosen non-family rows (identical training volume), plus `natural`. Estimands: CTK(d) = recall(d) - recall(0) and the increments 50 to 100 and 100 to 200, family-macro, federation-wide primary, own-domain secondary, 5% FPR primary.
- Controls: dose 0 versus the standard absent arm within +/-0.03; structural checks that the realised dose equals d at peers, 0 at the target, and training volume is constant.
- Statistics: paired seeds, BCa 9,999, exact Wilcoxon, Holm across levels, +/-0.03 equivalence for H-DOSE-2.
- Monotonicity (A1) is checked over every consecutive exact level (corrected in the final verification pass; the code had compared only the top and bottom levels; one secondary cell, replication set own-domain 1% FPR, changes to not met).
- Interpretation (fixed): onset plus equivalence at 100 to 200 = no further increase beyond about 100 samples; still rising past 200 = saturation not observed within the supported range; flat from small d = onset below 10.
- Result: pooled family sets, FedAvg, federation-wide, 5% FPR: CTK 0.031 at d = 10, 0.050 at 25, 0.075 at 50, 0.119 at 100 and 0.151 at 200 (all 10/10 seeds; d = 25 and above exceed +0.03 with interval lower bounds above it); the 100 to 200 increment is +0.032 (0.020, 0.046), not inside the band, so H-DOSE-2 is not met and saturation was not observed up to 200 exact samples. All structural consistency checks passed (0 mismatches) and dose 0 matches the absent arm (-0.000).

*EXT-CTRL (coherent placebo and robust aggregation, seeds 320-329).*
- Questions: Q1 is the gain specific to the hidden family or does an equal count of another coherent family help too? Q2 does robust aggregation erase rare-peer knowledge?
- Hypotheses: H-CTRL-1 placebo effect equivalent to zero (95% BCa inside +/-0.03) and CTK minus placebo effect above +0.03; H-CTRL-2 CTK under trimmed mean and under the coordinate-wise median each has a lower bound above +0.03 and is non-inferior to mean-aggregation CTK (lower bound of the difference above -0.03).
- Population and arms: frozen primary and replication sets; FedAvg peer-present, absent-everywhere, placebo (rows of a size-matched non-hidden family G, chosen by peer fit count closest to F, substituted at equal training volume); aggregation variants mean, trimmed mean (trim 1 of 4 per side) and median.
- Estimands: placebo effect, CTK per aggregator and CTK difference to mean; seed-paired, family-macro, federation-wide primary, own-domain secondary, 5% FPR.
- Statistics: BCa 9,999, exact one-sided Wilcoxon, +/-0.03 band, Holm across the two aggregators within H-CTRL-2.
- Interpretation (fixed): H-CTRL-1 met = family specificity supported; placebo effect positive outside the band = CTK wording narrowed. H-CTRL-2 not met = CTK depends on plain averaging and is scoped to FedAvg.
- Result (pooled sets, federation-wide, 5% FPR): placebo effect -0.001 (-0.008, 0.006), inside the band; CTK minus placebo +0.095 (0.079, 0.116); CTK by mean, trimmed mean and median 0.095, 0.088 (0.060, 0.112) and 0.126 (0.104, 0.145); trimmed minus mean -0.007 (-0.028, 0.009), median minus mean +0.032 (0.014, 0.047). H-CTRL-1 and H-CTRL-2 are both met at 5% FPR in the pooled set. Own-domain per-set intervals are wide (primary own-domain placebo effect -0.009, -0.043 to 0.009; H-CTRL-1 is not met for the primary own-domain 5% contrast) and in the replication set trimmed-mean CTK is attenuated (federation-wide 0.042 versus 0.054, own-domain 0.026 with an interval reaching -0.002). H-CTRL-2 is not met at 10% FPR (pooled) or in the replication set at any alpha; the full verdict table is `results/extension-b/controls-verdicts.csv`.

**Amendment A1 (recorded before any seed 310-329 run; no run directory or evaluation existed for those seeds).** Implementation clarifications only; no hypothesis, population, seed or estimand changed. (i) EXT-CTRL: with four clients a standard even-n median equals the trimmed mean above, so the second aggregator is the coordinate-wise lower median (`median-lower`); placebo rows are matched per peer to the hidden family's peer-present count with any shortfall reallocated to other peers (total count and volume unchanged, recorded as `reallocated`, results stratified by reallocated share); placebo family chosen by closeness of total peer fit count, largest hidden family first; Holm within H-CTRL-2 across the two aggregators, one-sided Wilcoxon; decision rules as in the hypotheses. (ii) EXT-DOSE: dose draws are nested across levels within a (seed, family) pair, dose 0 is its own arm; Holm over the six levels {10, 25, 50, 100, 200, natural}, one-sided Wilcoxon; H-DOSE-1 met when the highest exact level has Holm p below 0.05 and family-macro CTK is non-decreasing across levels; H-DOSE-2 met when the 100 to 200 increment is inside +/-0.03. Post-amendment hashes for the two files are in `FREEZE.md`.

*EXT-REP (representation replication, seeds 330-339; Amendment A2, frozen 2026-09-25 before any EXT-REP partition, feature cache or run; text in `docs/temp/extension-protocol/EXT-REP-representation-replication.md` and `FREEZE.md`).*

- Question: holding application identity, family identity, client assignment and split constant, do richer representations rescue families poorly detected under full exposure with LAMDA's 925 static features? This is NOT external validation: 81% of McNdroid applications are in LAMDA and the family labels come from the same AVClass2 run.
- Hypotheses: H-REP-1: family-macro seed-paired difference in central full-exposure federation-wide recall (5% FPR), McNdroid static minus LAMDA static, over the priority set P = {hiddad, gappusin, revmob}, has a 95% BCa lower bound above +0.03. H-REP-2 (formal no-meaningful-rescue equivalence hypothesis): each tested representation's hiddad upper bound is below +0.03. Secondary and descriptive: contrast set C = {leadbolt, airpush, dowgin}, adwo, call graph and JSON in P, and CTK by representation.
- Population: SHA-256-aligned overlap of LAMDA (named labels, existing four-client assignment) and all three McNdroid representations, 554,136 rows. LAMDA labels and clients are used for every representation; the identity-safe 60/20/20 partition is recomputed once per seed on the overlap and shared by all representations; the original primary family set under the original eligibility rule, dropped families reported without replacement.
- Representations: R0 LAMDA static (925, restricted to the overlap; baseline of this study), R1 McNdroid static (2,390), R2 call graph (2,793, standardised), R3 report-JSON (17,483, signed log1p, reduced to columns with at least 1% positive prevalence, standardised). Amendment A3 (final verification pass): every R2/R3 column selection and standardisation is fitted by each trained model from exactly its own training rows (local: that client; central and FedAvg: the pooled rows of that arm) and applied unchanged to its calibration and test rows; R2 and R3 were rerun with the frozen seeds. Original MLP pipeline, budgets and evaluation; only input dimension changes.
- Arms: local, central, FedAvg with peer-present, family-absent-everywhere and full-exposure, per representation (experiments `representation-r0` to `representation-r3`). Estimands: seed-paired central full-exposure recall differences versus R0 (per family, macro over P and C) and CTK per representation; federation-wide primary, own-domain secondary. Fresh seeds 330-339, mode `extension-b`. BCa 9,999, exact Wilcoxon, Holm over the nine P-by-representation tests, band +-0.03.
- Pre-stated interpretation: both met, representation limits are family-specific; H-REP-1 met and H-REP-2 not met, "some representation rescues hiddad"; H-REP-1 not met, the original wording stands. Nothing alters the original representation-limited gate or 9/0/3. Development-seed harness numbers motivated the run and are not evidence. Corrected in the final verification pass (logic only): the "H-REP-2 not met" branch now distinguishes rescue shown (some hiddad lower bound above +0.03), no meaningful rescue (the frozen H-REP-2) and unresolved; failing the no-rescue criterion is not evidence of rescue.
- Result after Amendment A3 (per-arm training-only R2/R3 preprocessing, rerun with the frozen seeds; `results/extension-b/representation-effects.csv`, `representation-verdicts.csv`; federation-wide, 5% FPR, difference versus LAMDA static): H-REP-1 met, McNdroid static priority-macro {hiddad, gappusin, revmob} +0.082 (0.053, 0.132), positive in 10/10 seeds; gappusin +0.146 (0.051, 0.282); revmob +0.086 (-0.001, 0.167); hiddad +0.013 (-0.046, 0.159), 3/10 positive seeds; adwo (exploratory) +0.328; contrast-set macro -0.029; call graph priority-macro +0.006 (-0.035, 0.036), hiddad -0.055 (-0.103, -0.023); report-JSON priority-macro -0.132 (-0.176, -0.064), hiddad -0.164 (-0.248, -0.102). H-REP-2 formally not met (static hiddad upper bound 0.159); hiddad status unresolved (`rep1-met-hiddad-unresolved`), not a rescue. FedAvg CTK priority-macro: R0 0.167, R1 0.237, R2 0.206, R3 0.108 (R2 minus R0 +0.039, 0.022 to 0.064). Representation limits are family-specific; class D; original outcome unchanged.
- Limitations: the overlap is not independent of LAMDA; provider-side feature selection cannot be reconstructed train-only; dimensionality differs across representations.

**Retraction.** An earlier revision of this section stated that the McNdroid representation replication was not run because a development-seed proof of concept did not justify it, and attributed part of that to the report-JSON zero recall in the POC log. That statement is superseded: the POC harness pooled the decision threshold, and a per-client-threshold harness (as in the original pipeline) supported running the study, which was then frozen as A2 and run.

**Extension studies not carried out.** A family-aware auxiliary training objective, missing-class-aware baselines (FedRS, FedLC, FedVLS and others; inapplicable to a binary detector with the family inside the positive class) and independent external datasets (KronoDroid MARGINAL; McNdroid not independent, see EXT-REP) were not added.
...
The strongest neighboring bodies of work include federated rare-class transfer, class-incremental intrusion detection, prototype exchange for disjoint/rare classes, and Android malware FL under non-IID or drift. A second, deeper collision audit (`docs/Research Evidence.md`, section 29) applies an identification-based rule to 41 rows and finds 0 DIRECT, 4 PARTIAL (CyberForce, CELEST, FLEKD-IDS, and borderline Shao/Otani), 34 ADJACENT and 3 NONE. Popoola et al. was reclassified ADJACENT after its accepted-version manuscript showed that the local comparison is aggregate rather than missing-class-specific. Papers read in full or main text include FedRS, FedLMD, FedCKD, Ciaramella et al., Liu et al. Cybersecurity 2026 and the PLOS One 2026 zero-day ensemble; FID-SPA and FEDroid full texts remain unavailable. The audit used partial forward-citation chaining and a name-based security-venue sweep. The four PARTIAL papers show local-versus-federated or peer-exposure contrasts on a deliberately missing class; vacant-class evaluation, label-set-size manipulation at fixed sample counts and Android malware FL with family skew are adjacent. CyberForce's unmatched absent-everywhere arm and peer-holder sweep mean those arms are not new; the volume-matched decomposition is. Locally missing classes, family-skewed Android malware FL, exposure sweeps and natural-versus-synthetic partitions each have precedent, so the dose analysis and the natural-scarcity check are not claimed as independent contributions. The combined measurement design (controlled hidden-family exposure, matched family-absent-everywhere control, full-exposure reference, pooling-versus-complementary decomposition with own-domain and worst-client evaluation) was not found in the audited literature. The audit is limited (proceedings of the main security venues are not reliably indexed; many primary sources unread) and does not replace the pre-submission citation-chaining audit.
...
### Extension status and remaining limits

- Representation replication (EXT-REP): done, class D; H-REP-1 met, H-REP-2 formally not met because the McNdroid-static hiddad interval is wide; hiddad status unresolved (no rescue shown; call graph and report-JSON exclude one). Limits: the SHA-aligned overlap is not independent of LAMDA, provider-side feature selection cannot be reconstructed train-only, and dimensionality differs across representations. The representation-limited findings remain bounded to the tested representations.
- Larger-family explanation (EXT-LFAM): done, class D; rho 0.39 (0.06, 0.65) over 32 families is a moderate, wide-interval association, not a causal explanation, and not robust to restricting to the 27 always-eligible families (rho 0.27, interval spanning zero); the original gate stays "rejected".
- Dose (EXT-DOSE): CTK rises with exact peer exposure through 200 samples and is consistent with diminishing marginal returns, but no plateau is observed; the primary-set half-effect dose is about 53 samples and the replication set's is not identified (descriptive fits, Research Evidence 33.4). Families need very different exposure (10 samples to non-response at 200).
- Diagnostics (class C on extension data, `diagnostics --mode extension-b`, Research Evidence 33.4): EXT-REP McNdroid-static gains keep lower bounds above zero at exactly 5% test FPR; EXT-LFAM rho is not driven by any single family but partly carried by high-leverage families with incomplete eligibility; no placebo stratum shows a materially positive effect (largest client-level point estimate about +0.014; pooled federation-wide -0.001, -0.008 to 0.006), and peer reallocation does not materially change the result; rare-peer erasure by robust aggregation is not supported.
- Placebo and robust aggregation (EXT-CTRL): own-domain per-set control intervals are wide (some own-domain per-set contrasts do not meet the frozen rules), and in the replication set trimmed-mean CTK is attenuated relative to the mean.
